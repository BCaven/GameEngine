...
def write_bcf_data(filepath):
    ...
    for tri in me.loop_triangles:
        for vert_index in tri.vertices:
            ...
            float_vertices.extend(v.co[0], v.co[1], v.co[2])
            float_normals.extend(v.normal[0], v.normal[1], v.normal[2])
    ...
    print(f"Num vertices: {len(float_vertices / 3)}")
    f = open(filepath, "wb", encoding='utf-8')
    
    f.write(f"{len(float_vertices /3 )} ")
